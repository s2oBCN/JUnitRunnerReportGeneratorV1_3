...
import com.junit.tools.testcaserunner.JUnitTestCaseRunner;

/**
 * This utility class was based originally on <a href="mailto:">Daniel Le
 * Berre</a>'s <code>RTSI</code> class. This class can be called in different
 * modes, but the principal use is to determine what subclasses/implementations
 * of a given class/interface exist in the current runtime environment.
 * 
...
public class ClassFinder {
	...
	public final Map<URL, String> getClasspathLocations() {
		...
		// System.err.println(pathSep+"  "+classpath);
		// String classpath =
		// "E:/SVN/Frameworks/hw-framework-exception/target/test-classes";
	...
	private void includeJar(File file, Map<URL, String> map) {
		...
		if(!Boolean.parseBoolean(AppConfig.getProperty("lookjarstoofortestcases").trim()))
			return;
		if (file.isDirectory())
			return;
	...
	public Vector<Class<?>> getAllTestCases() {
		...
		v = finder.findSubclasses("junit.framework.TestCase");
